...
class SimpleNLMR_3(base.BaseEstimator):
	def __init__(self, state_dim = 100, h2_layer_dim = 300, initial_feature_scaling_factor = 0.1, initial_contraction_coefficient = 0.99, is_esn = False, bttp_truncation = -1, optimizer_opts = {}, tr_l2_reg = 0.0, ta_l2_reg = 0.0, rf_l2_reg = 0.0, rec_l2_reg = 0.0, initial_state_l2_reg = 0.0, h2_l2_reg = 0.0, dtype = None, random_state = 0, verbose = 0):
	...
	def make_model(self):
		...
		self.train_params_list_.append({'var': self.W_rec_to_h2_, 'sparse_updates': True, 'l2_regularization' : self.h2_l2_reg})
		...
		# Raw target score
		def get_raw_target_score_graph(X_state, Target_embeddings, Readout_features_partial_activation):
			Prereadout = tt.dot(X_state * Target_embeddings, self.W_rec_to_h2_) + Readout_features_partial_activation + self.B_to_h2_
			return tt.dot(tt.tanh(Prereadout), self.W_h2_to_out_).flatten() + self.B_to_out_[0]
			
				
		self.Raw_target_score_ = get_raw_target_score_graph(self.X_state_, self.Target_embeddings_, self.Readout_features_partial_activation_)
		...
		self.Raw_target_score_with_repeated_state_ = get_raw_target_score_graph(X_state_repeated, self.Target_embeddings_, self.Readout_features_partial_activation_)
		...
		def get_loss_graph(X_in_tr_seq, X_in_tas_seq, X_in_res_seq, X_in_ids_seq, X_state_0 = None):
			...
			def seq_loss_seq_scan_proc(K, X_in_id, Nte, X_state_tm1, I_tm1):
				...
				Prereadout = tt.dot(X_state_tm1 * Target_embeddings_seq[I_tm1:I], self.W_rec_to_h2_) + Readout_features_partial_activation + self.B_to_h2_
		...
		def get_train_loss_graph(X_in_tr_seq, X_in_tas_seq, X_in_res_seq, X_in_ids_seq, X_state_0 = None):
			...
			(_, _, Loss_seq), _ = theano.scan(seq_tloss_seq_scan_proc, sequences=[tt.arange(Seq_len), X_in_ids_seq, Nte_seq], outputs_info = [X_state_0, np.int64(0), None], non_sequences=[Readout_features_partial_activation_seq, Target_embeddings_seq, Transition_embeddings_seq_ext], truncate_gradient = self.bttp_truncation, name = 'seq_tloss_seq_scan')
			...
			return Mean_Loss, gr_MTL_wrt_W_rf_to_h2, gr_MTL_wrt_W_ta_to_hidden_, gr_MTL_wrt_W_tr_to_hidden_, gr_MTL_wrt_B_to_hidden_
			
		
		self.Train_Loss_, gr_MTL_wrt_W_rf_to_h2, gr_MTL_wrt_W_ta_to_hidden_, gr_MTL_wrt_W_tr_to_hidden_, gr_MTL_wrt_B_to_hidden_ = get_train_loss_graph(self.X_in_tr_seq_, self.X_in_tas_seq_, self.X_in_res_seq_, self.X_in_ids_seq_)
