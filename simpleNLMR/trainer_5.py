...
def getDepSentenceFeatures(sid, sent, dtype, trFM, taFM, reFM, pfFM):
	...
	pfsSeq = pfFM.transform_seq_to_indices(pfsL)
	pidsSeq = np.array(pidsL, dtype=np.int32)
	return (trSeq, tasSeq, resSeq, vidSeq, pfsSeq, pidsSeq)

def trainDepSentence(sid, sent, snlmr, trFM, taFM, reFM, pfFM):
	return snlmr.Train_proc_(*getDepSentenceFeatures(sid, sent, snlmr.dtype_, trFM, taFM, reFM, pfFM))
...
def prepareValidationSet(validSentF, dtype, trFM, taFM, reFM, pfFM, verbose = 0):
	rv = []
	for i, sent in enumerate(readDepSentences(validSentF)):
		if len(sent) < 3:
			...
			continue
		rv.append(getDepSentenceFeatures(i+1, sent, dtype, trFM, taFM, reFM, pfFM))
		if verbose > 2:
...
def train(trainSentF, validSentF, outParamsF, trFM, taFM, reFM, pfFM, state_dim = 100, patience = 5, loss_averaging_factor = 0.1, valid_freq = 300, cutoff_loss = 0.0, verbose = 0):
	snlmr = SimpleNLMR_4(state_dim = state_dim, optimizer_opts={'clip_nabla' : (-1.0, 1.0), 'verbose' : 3}, tr_l2_reg = 0.0001, ta_l2_reg = 0.0001, rf_l2_reg = 0.01, rec_l2_reg = 0.01, initial_state_l2_reg = 0.01, h2_l2_reg = 0.01, random_state = 0, verbose = 3)
	snlmr.fit(transition_features_map = trFM, target_features_map = taFM, readout_features_map = reFM, path_features_map = pfFM, enable_training = True)
	
	if verbose > 0:
		print >> sys.stderr, "Preparing validation set"
	validationSet = prepareValidationSet(validSentF, snlmr.dtype_, trFM, taFM, reFM, pfFM, verbose)
	...
	while live:
		...
		for i, sent in enumerate(readDepSentences(trainSentF)):
			...
			if len(sent) < 3:
				...
				total_sentences += 1
				continue
			training_loss = trainDepSentence(i+1, sent, snlmr, trFM, taFM, reFM, pfFM)
			total_sentences += 1
...
def main():
	...
	train(trainSentF, validSentF, outParamsF, trFM, taFM, reFM, pfFM, state_dim = 100, valid_freq = 500, patience = 4, cutoff_loss = 0.0, verbose = 3)
